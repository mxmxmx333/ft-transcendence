...
async function getChatHistory()
{
	...
	if (chats.length > 0)
	{
		...
		DOM.chatHistory.appendChild(DOM.noChats);
}
...
function RegisterSocketListeners()
{
	...
	chatSocket.on("your request is accepted", (target_id: number) => {
		...
		if (request)
			request.remove();
		if (!DOM.requestsList.hasChildNodes())
			DOM.requestsList.appendChild(DOM.noRequests);
	});
	...
	chatSocket.on("got removed from friends", (by_id: number) => {
		...
		if (chat)
			chat.remove();
		if (!DOM.chatHistory.hasChildNodes())
			DOM.chatHistory.appendChild(DOM.noChats);
		...
		if (friend)
			friend.remove();
		if (!DOM.friendsList.hasChildNodes())
			DOM.friendsList.appendChild(DOM.noFriends);
		...
		if (blocked)
			blocked.remove();
		if (!DOM.blockedList.hasChildNodes())
			DOM.blockedList.appendChild(DOM.noBlocked);
	});
