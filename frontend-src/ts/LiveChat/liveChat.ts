...
import {
  convertISOtoLocaleHM,
  displayDateTag,
  displayMessage,
  lastMsgDateISO,
  manageChatFooter,
  manageInviteBtnStatus,
  openChat,
  updateLastMsgDateISO,
} from './liveChatRS';
...
export function displayLiveChat() {
  if (chatSocket.lostConnection()) return;
...
DOM.searchBar.addEventListener('focusout', (e) => {
  ...
  if (activeMenu.match('Chats')) DOM.chatHistory.classList.remove('hidden');
  if (activeMenu.match('Friends')) {
    if (activeExMenu.match('All')) DOM.friendsList.classList.remove('hidden');
    else if (activeExMenu.match('Blocked')) DOM.blockedList.classList.remove('hidden');
    else DOM.requestsList.classList.remove('hidden');
  }
  if (activeMenu.match('Users')) DOM.findUser.classList.remove('hidden');
});
...
DOM.searchBar.addEventListener('input', () => {
  ...
  if (activeMenu.match('Friends')) {
    list = activeExMenu.match('All')
      ? DOM.friendsList.querySelectorAll('li')
      : activeExMenu.match('Blocked')
        ? DOM.blockedList.querySelectorAll('li')
        : DOM.requestsList.querySelectorAll('li');

    // To display back the list that was active, when search bar is empty
    currentMenuContent = activeExMenu.match('All')
      ? DOM.friendsList
      : activeExMenu.match('Blocked')
        ? DOM.blockedList
        : DOM.requestsList;
  ...
  list.forEach((element) => {
    if (
      element.dataset.username?.toLowerCase().match(DOM.searchBar.value.toLowerCase()) &&
      DOM.searchBar.value !== ''
    ) {
  ...
  if (!matchFound) {
    DOM.searchBar.value === ''
      ? (DOM.searchResults.classList.add('hidden'), currentMenuContent.classList.remove('hidden'))
...
function addElementToList(
  ...
  profileName.className =
    'ml-3 font-bold text-[15px] max-w-40 whitespace-nowrap overflow-hidden text-ellipsis';
  profileName.innerHTML = name;
  notificationIcon.className =
    'notification-dot w-3 h-3 ml-auto mr-3 rounded-full notification-dot-color';
  ...
  if (list === DOM.requestsList && status === 'not viewed') wholeBox.appendChild(notificationIcon);
...
export function addChatHistory(
  ...
  profileName.className =
    'ml-3 font-bold text-[15px] whitespace-nowrap overflow-hidden text-ellipsis';
  msgPreview.className =
    'msg-preview ml-3 text-[13px] -mt-1 whitespace-nowrap overflow-hidden text-ellipsis';
  profileName.innerHTML = name;
  msgPreview.innerHTML = lastMsg;
  notificationIcon.className =
    'notification-dot flex h-[18px] min-w-[18px] max-w-fit ml-auto mr-2 p-1 justify-center items-center rounded-full text-xs font-bold notification-dot-color';
  ...
  if (type === 'invitation') msgPreview.classList.add('text-yellow-300', 'italic', 'font-semibold');
  if (target_id === tournamentID) {
    div.className =
      'w-9 h-9 flex justify-center items-center text-yellow-400 text-2xl font-bold rounded-full bg-slate-700';
    div.innerHTML = 'T';
    wholeBox.appendChild(div);
  } else {
    wholeBox.appendChild(div);
  ...
  if (option === 'prepend') DOM.chatHistory.prepend(chatElement);
  else DOM.chatHistory.appendChild(chatElement);
...
DOM.requestsList.addEventListener('mousedown', (e) => {
  ...
  if (!(target && target.dataset.username) || !DOM.requestsList.contains(target)) return;
...
DOM.searchList.addEventListener('mousedown', (e) => {
  ...
  if (activeMenu.match('Friends')) {
    ...
    if (activeExMenu.match('All')) handleUserKinds();
    else if (activeExMenu.match('Blocked')) showOptions(DOM.friendsMenuExBlockedOptions);
    else {
      DOM.requesterID.innerHTML = target.dataset.username;
      showOptions(DOM.friendsMenuExRequestsOptions);
...
async function searchUsers(input: string) {
  ...
  } else DOM.noResults.classList.remove('hidden');
}
...
async function getChatHistory() {
  ...
  } else DOM.chatHistory.appendChild(DOM.noChats);
}
...
export async function getFriendsAll() {
  ...
  } else DOM.friendsList.appendChild(DOM.noFriends);
}
...
export async function getFriendsBlocked() {
  ...
  } else DOM.blockedList.appendChild(DOM.noBlocked);
}
...
async function getFriendsRequests() {
  ...
  } else DOM.requestsList.appendChild(DOM.noRequests);
}
...
async function handleUserKinds() {
  ...
  if (status === 'mutual block' || status === 'target blocked')
    showOptions(DOM.friendsMenuExBlockedOptions);
  else if (status === 'blocked by target') showOptions(DOM.blockedByUserOptions);
  else if (status === 'no blocks') showOptions(DOM.friendsMenuOptions);
  else if (status === 'not friends') {
    const requested = DOM.requestsList.querySelector(
      `li[data-id="${currentTargetID}"]`
...
function RegisterSocketListeners() {
  ...
  chatSocket.on('your request is accepted', (target_id: number) => {
    ...
    if (currentTargetID === target_id) showOptions(DOM.friendsMenuOptions);

    getFriendsAll();

    const request = DOM.requestsList.querySelector(
      `li[data-id="${currentTargetID}"]`
    ) as HTMLLIElement | null;
    if (request) request.remove();
    if (!DOM.requestsList.hasChildNodes()) DOM.requestsList.appendChild(DOM.noRequests);
  });
  ...
  chatSocket.on('got removed from friends', (by_id: number) => {
    ...
    if (chat) chat.remove();
    if (!DOM.chatHistory.hasChildNodes()) DOM.chatHistory.appendChild(DOM.noChats);

    // Remove from friends list
    const friend = DOM.friendsList.querySelector(`li[data-id="${by_id}"]`) as HTMLLIElement | null;
    if (friend) friend.remove();
    if (!DOM.friendsList.hasChildNodes()) DOM.friendsList.appendChild(DOM.noFriends);

    // Remove from blocked list
    const blocked = DOM.blockedList.querySelector(`li[data-id="${by_id}"]`) as HTMLLIElement | null;
    if (blocked) blocked.remove();
    if (!DOM.blockedList.hasChildNodes()) DOM.blockedList.appendChild(DOM.noBlocked);
  });
  ...
  chatSocket.on('you got unblocked', (by_id: number) => {
    ...
    if (currentTargetID === by_id) {
      // If chat window is open
      if (!DOM.chatMsgArea.classList.contains('hidden')) manageChatFooter();
  ...
  chatSocket.on(
    'received message',
    (from_id: number, name: string, msg: string, timeDbFormat: string) => {
      ...
      if (currentTargetID === from_id) {
        ...
        let prevMsgType = bottomMsg
          ? bottomMsg.classList.contains('self-end')
            ? 'sent'
            : 'received'
          : 'none';

        if (prevMsgType === 'none')
          // Very first message in chat (lastMsgDateISO is empty)
          displayDateTag(timeDbFormat, timeDbFormat, 'append', true);
        else
          prevMsgType = displayDateTag(lastMsgDateISO, timeDbFormat, 'append')
            ? 'none'
            : prevMsgType;
        ...
        if (DOM.goToBottomIcon.classList.contains('opacity-100')) {
          DOM.newMsgsCount.innerHTML =
            msgsOnScrollCount === 99 ? '99+' : String(++msgsOnScrollCount);
          DOM.newMsgsCount.classList.remove('hidden');
          DOM.goToBottomIcon.classList.remove('pointer-events-none');
        } else DOM.chatMsgArea.scrollTop = DOM.chatMsgArea.scrollHeight;
  ...
  chatSocket.on(
    'update notification',
    (from_id: number, name: string, lastMsgPreview: string, unread_amount: number) => {
      if (activeMenu !== 'Chats') DOM.chatsNotification.classList.remove('hidden');
      if (window.location.pathname !== '/livechat')
        DOM.liveChatNotification.classList.remove('hidden');

      const target = DOM.chatHistory.querySelector(
        `li[data-id="${from_id}"]`
      ) as HTMLLIElement | null;
      if (!target) addChatHistory(name, null, lastMsgPreview, from_id, unread_amount, 'prepend');
      else {
        const notification = target.querySelector('.notification-dot') as HTMLDivElement;
  ...
  chatSocket.on('received game invitation', (from_id: number, nickname: string) => {
    if (currentTargetID === from_id) {
      ...
      // It could be that it's an open chat from newly added friend (so no chat history yet)
      if (!currentChat) addChatHistory(nickname, null, 'Game Invitation', from_id, 0, 'prepend'); // In this function I check if it was tournamentID
  ...
  chatSocket.on('update notification - game', (from_id: number, name: string) => {
    ...
    if (!target) addChatHistory(name, null, 'Game Invitation', from_id, 0, 'prepend', 'invitation');
    else {
      const notification = target.querySelector('.notification-dot') as HTMLDivElement;
      // notification.classList.remove("hidden");
      // notification.innerHTML = unread_amount > 99 ? "99+" : String(unread_amount);
  ...
  chatSocket.on('invitation declined', (by_id: number) => {
    if (currentTargetID === by_id) {
      ...
      if (inviteBtnCurrent) {
        ...
        if (btnInfo) inviteBtnCurrent.removeChild(btnInfo);
        inviteBtnCurrent.classList.remove(
          'text-yellow-300',
          'text-opacity-50',
          'pointer-events-none'
        );
        inviteBtnCurrent.classList.add(
          'neon-text-yellow',
          'neon-border-pink',
          'hover:neon-bg-pink',
          'hover:text-black'
  ...
  chatSocket.on('invitation canceled', (by_id: number) => {
    if (currentTargetID === by_id)
      // doesn't matter if in chat, but cannot hide someone elses
      DOM.gameInviteBanner.classList.add('hidden');
  });
}
