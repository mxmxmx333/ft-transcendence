...
export function updateLastMsgDateISO(update: string) {
  lastMsgDateISO = update;
}
...
const closeChatsOptionsBtns =
  document.querySelectorAll<HTMLButtonElement>('.close-chat-options-btn');
...
const removeFromFriendsBtns = document.querySelectorAll<HTMLButtonElement>(
  '.remove-from-friends-btn'
);
...
export function displayMessage(
  ...
  if (type === 'received') {
    chatBubble.className =
      'bg-cyan-600 rounded-lg py-0.5 px-2 max-w-xs w-fit break-words hyphens-auto whitespace-pre-wrap';
    if (prevMsgType === 'sent') {
    ...
    chatBubble.className =
      'self-end bg-pink-600 rounded-lg py-0.5 px-2 max-w-xs w-fit break-words hyphens-auto whitespace-pre-wrap';
  ...
  if (method === 'prepend') DOM.chatMsgArea.prepend(chatBubble);
  else DOM.chatMsgArea.appendChild(chatBubble);
}
...
DOM.chatInput.addEventListener('input', () => {
  ...
  if (
    DOM.chatMsgArea.scrollHeight - (DOM.chatMsgArea.scrollTop + DOM.chatMsgArea.clientHeight) <
    25
  )
...
DOM.sendMsgBtn.addEventListener('click', async () => {
  ...
  if (!target)
    // Not in chat history yet --> New conversation
  ...
  else prevMsgType = displayDateTag(lastMsgDateISO, time, 'append') ? 'none' : prevMsgType;
...
async function removeFromFriends() {
  ...
  const chat = DOM.chatHistory.querySelector(
    `li[data-id="${currentTargetID}"]`
  ) as HTMLLIElement | null;
  if (chat) chat.remove();
  ...
  const friend = DOM.friendsList.querySelector(
    `li[data-id="${currentTargetID}"]`
  ) as HTMLLIElement | null;
  ...
  const blocked = DOM.blockedList.querySelector(
    `li[data-id="${currentTargetID}"]`
  ) as HTMLLIElement | null;
...
async function blockFriend() {
  ...
  if (
    currentOptionsWindow === DOM.chatsMenuOptions ||
    currentOptionsWindow === DOM.chatsMenuBlockedByOptions
  )
    showOptions(DOM.chatsMenuBlockedOptions);
  else showOptions(DOM.friendsMenuExBlockedOptions);
  chatSocket.removeInvitation('sent');
}
...
async function checkForBlockedByAfterUnblock() {
  ...
  if (status === 'blocked by target') {
    ...
    else showOptions(DOM.blockedByUserOptions);
  } else {
    if (currentOptionsWindow === DOM.chatsMenuBlockedOptions) showOptions(DOM.chatsMenuOptions);
    else showOptions(DOM.friendsMenuOptions);
  }
}
...
async function checkBlocksForOptionsBtn() {
  ...
  if (status === 'no blocks') showOptions(DOM.chatsMenuOptions);
  else if (status === 'mutual block' || status === 'target blocked')
    showOptions(DOM.chatsMenuBlockedOptions);
  else showOptions(DOM.chatsMenuBlockedByOptions);
}
...
export function displayDateTag(
  time1: string,
  time2: string,
  method: string,
  afterLoadOrFirst: boolean = false
): boolean {
  // Same day - no need for adding date tag
  if (time1.split('T')[0] === time2.split('T')[0] && afterLoadOrFirst === false) return false;

  const dateTag = document.createElement('div');

  // After last loaded message - prepend. Will remove in onChatScroll() if needed.
  if (afterLoadOrFirst)
    // This will be on very top so no mt-8, anyway msgs container has padding
    dateTag.className =
      'self-center w-fit mb-4 bg-black text-yellow-300 text-opacity-90 rounded-full px-4 text-sm font-semibold justify-center items-center';
  else
    dateTag.className =
      'self-center w-fit mt-10 mb-4 bg-black text-yellow-300 text-opacity-90 rounded-full px-4 text-sm font-semibold justify-center items-center';
...
async function checkGameInvitations() {
  ...
  } else DOM.gameInviteBanner.classList.add('hidden');
}

export async function manageInviteBtnStatus(
  nextOptionsWindow: HTMLElement,
  onOnlineStatusChange: string = ''
) {
  ...
  if (inviteBtnNext) {
    const status = await chatSocket.recordOrCheckGameInvitation('check');
    if (status === 'error') return alert('Something went wrong. Please try again later');
    if (status === 'can send') return; // No change needed
    if (status === 'offline') {
      inviteBtnNext.classList.remove(
        'neon-text-yellow',
        'neon-border-pink',
        'hover:neon-bg-pink',
        'hover:text-black'
      ...
      btnInfo.className =
        'text-yellow-300 text-opacity-50 italic justify-center -mt-1 flex text-sm font-normal';
...
async function onChatScroll() {
  ...
  if (
    DOM.chatMsgArea.scrollTop <
    DOM.chatMsgArea.scrollHeight - DOM.chatMsgArea.clientHeight - 50
  ) {
  ...
  if (DOM.chatMsgArea.scrollTop > 200 || nothingMoreToLoad)
    // Not near the top or no more msg to load
    return;
  ...
  let prevMsgType = DOM.chatMsgArea.children[1].classList.contains('self-end')
    ? 'sent'
    : 'received';
...
async function sendGameInvitation(inviteBtn: HTMLButtonElement) {
  ...
  if (invitationStatus !== 'success')
    return alert(
      invitationStatus === 'offline'
        ? 'This user is currently offline'
        : invitationStatus === 'already sent'
          ? 'You have already invited this user'
          : invitationStatus === 'already received'
