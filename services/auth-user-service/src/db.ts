...
`);
	...
	db.exec(`CREATE INDEX IF NOT EXISTS idx_messages ON messages(conversation_id, created_at DESC)`);
	db.exec(`CREATE INDEX IF NOT EXISTS idx_tournament_msgs ON tournament_msgs(receiver_id, created_at DESC)`);
