import { Constants, Action } from './game_old';
...
class ImprovedNeuralNetwork {
  ...
  private initializeWeights() {
    ...
    this.weights1 = this.createMatrix(this.inputSize, this.hiddenSize1, () => (Math.random() - 0.5) * 2 * xavierScale1);
    this.weights2 = this.createMatrix(this.hiddenSize1, this.hiddenSize2, () => (Math.random() - 0.5) * 2 * xavierScale2);
    this.weights3 = this.createMatrix(this.hiddenSize2, this.outputSize, () => (Math.random() - 0.5) * 2 * xavierScale3);
  ...
  private createMatrix(rows: number, cols: number, initializer: () => number): number[][] {
    return Array(rows).fill(0).map(() => Array(cols).fill(0).map(initializer));
  }
  ...
  private backpropagate(state: number[], action: number, error: number, lr: number) {
    ...
    // Update b3[action]
    this.momentumBias3[action] = momRate * this.momentumBias3[action] + (1 - momRate) * (lr * delta3);
...
export class ImprovedReinforcementLearningAI {
  ...
  private predictiveHorizon = 5; // Frames ahead to predict
  ...
  private normalizeGameState(state: GameState): number[] {
    const { ballX, ballY, ballVX, ballVY, aiY, playerY, canvasWidth, canvasHeight, ballSpeed, gameTime } = state;
    ...
    const timeToReachAI = ballVX > 0 ? (canvasWidth - this.constants.paddleWidth - this.constants.ballRadius - ballX) / Math.max(ballVX, 1e-3) : 999; // Zeit bis Ball AI-Seite erreicht
    ...
    const distanceToOptimal = Math.abs((aiY + this.constants.paddleHeight/2) - predictedBallY) / canvasHeight;
    const relativeSpeed = ballSpeed / this.constants.MAX_BALL_SPEED;
    const gameProgress = Math.min(gameTime / 30000, 1); // Normalize to 30 seconds: sagt der RL‑Policy damit, ob wir eher am Anfang (≈0) oder später (≈1) in der Runde sind. So kann sie z.B. später aggressiver spielen. Passt, wenn ein Spiel normalerweise so um die 30 sek dauert. Ändern falls anders! Potentiell auch einfach löschen / rausnehmen!
  ...
  private predictBallPosition(state: GameState, frames: number): number {
  ...
  private calculateAdvancedReward(state: GameState, action: number, nextState: GameState): number {
    let reward = 0;
    ...
    // Defensive Bonus
    if (state.ballVX < 0 && Math.abs(nextState.ballY - (nextState.aiY + this.constants.paddleCenter)) < 30) {
    ...
    // Bewegungseffizienz
    if (action === Action.Stay && ballDistance < 20) { // STAY when close
      reward += 0.05;
    }
  ...
  getAction(gameState: GameState): Action {
    ...
    if (this.lastState !== null) { // ist NULL zu Beginn
      const reward = this.calculateAdvancedReward(this.denormalizeState(this.lastState, gameState), this.lastAction, gameState);
    ...
    if (this.consecutiveLosses > 3) { // ist 0 zu Beginn
    ...
    if (Math.random() < currentEpsilon) {
      ...
      const qValues = this.network.forwardSoft(normalizedState);
      const ballDistance = Math.abs(gameState.ballY - (gameState.aiY + this.constants.paddleCenter));

      if (ballDistance < gameState.canvasWidth * 0.01) { // anpassen vielleicht?
  ...
  private denormalizeState(normalizedState: number[], currentState: GameState): GameState {
    return {
