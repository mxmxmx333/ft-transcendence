...
function onGetOnlineStatus(socket: Socket) {
  socket.on('get online status', (target_id: number, callback: (status: string) => void) => {
    if (activeUsers.get(target_id)) callback('Online');
...
function onGetFriends(socket: Socket, userID: number) {
  socket.on('search in friendships', (callback: (friends: any[]) => void) => {
    try {
      ...
      const pairs = stmt.all(userID, userID) as { user1_id: number; user2_id: number }[];
...
function onRecordRequest(socket: Socket, userID: number) {
  socket.on('record request', (target_id: number, callback: (status: string) => void) => {
    try {
      ...
      if (receiver) receiver.socket.emit('received request', userID);
      callback('success');
    } catch (err) {
...
function onAcceptRequest(socket: Socket, userID: number) {
  socket.on('accept friend request', (from_id: number, callback: (status: string) => void) => {
    try {
      ...
      if (receiver) receiver.socket.emit('your request is accepted', userID);
      callback('success');
    } catch (err) {
      console.error('[DB] Error: ', err);
      callback('error');
...
function onDeclineRequest(socket: Socket, userID: number) {
  socket.on('decline friend request', (from_id: number, callback: (status: string) => void) => {
    try {
      ...
      // Notify other side if active
      const receiver = activeUsers.get(from_id);
      if (receiver) receiver.socket.emit('your request is declined', userID);
      callback('success');
    } catch (err) {
      console.error('[DB] Error: ', err);
      callback('error');
...
function onRemoveFriend(socket: Socket, userID: number) {
  ...
  socket.on('remove friend', (target_id: number, callback: (status: string) => void) => {
    try {
      ...
      const receiver = activeUsers.get(target_id);
      if (receiver) receiver.socket.emit('got removed from friends', userID);
      callback('success');
    } catch (err) {
      console.error('[DB] Error: ', err);
      callback('error');
...
function onBlockUser(socket: Socket, userID: number) {
  socket.on('block user', (target_id: number, callback: (status: string) => void) => {
    try {
      ...
      const receiver = activeUsers.get(target_id);
      if (receiver) receiver.socket.emit('you got blocked', userID);
      callback('success');
    } catch (err) {
      console.error('[DB] Error: ', err);
      callback('error');
...
function onUnblockUser(socket: Socket, userID: number) {
  socket.on('unblock user', (target_id: number, callback: (status: string) => void) => {
    try {
      ...
      const receiver = activeUsers.get(target_id);
      if (receiver) receiver.socket.emit('you got unblocked', userID);
      callback('success');
    } catch (err) {
      console.error('[DB] Error: ', err);
      callback('error');
  ...
  socket.on('check blocks', (target_id: number, callback: (status: string) => void) => {
    try {
      ...
      const stmt = server.db.prepare(`
				...
				FROM lc_friendships
				WHERE user1_id = ? AND user2_id = ?`);
      const raw_result = stmt.get(user1_id, user2_id) as {
        user1_id: number;
        user2_id: number;
        user1_blocked: number;
        user2_blocked: number;
      };
      ...
      if (result.user1_blocked === false && result.user2_blocked === false) callback('no blocks');
      ...
      else callback(result.user2_id === userID ? 'blocked by target' : 'target blocked');
    } catch (err) {
      console.error('[DB] Error: ', err);
      callback('error');
    }
...
function onLoadMoreMessages(socket: Socket, userID: number) {
  socket.on(
    'load more messages',
    (target_id: number, lastLoaded: string, callback: (messages: any[]) => void) => {
      try {
        const user1_id = Math.min(userID, target_id);
        const user2_id = Math.max(userID, target_id);
        const stmt = server.db.prepare(`
...
function onRecordMessage(socket: Socket, userID: number) {
  socket.on(
    'record message',
    (target_id: number, msg: string, timeDbFormat: string, callback: (status: string) => void) => {
      try {
        const user1_id = Math.min(userID, target_id);
        const user2_id = Math.max(userID, target_id);
        const stmt = server.db.prepare(`
				...
				WHERE f.user1_id = ? AND f.user2_id = ?`);
        const raw_f_result = stmt.get(userID, user1_id, user2_id) as {
          nickname: string;
          convo_id: number;
          user1_id: number;
          user2_id: number;
...
function onLoadMoreTournamentMessages(socket: Socket, userID: number) {
  socket.on(
    'load more tournament messages',
    (lastLoaded: string, callback: (tmsgs: any[]) => void) => {
      try {
        const stmt = server.db.prepare(`
				SELECT message, created_at
				FROM tournament_msgs
...
function onRecordTournamentMessage(socket: Socket, userID: number) {
  socket.on(
    'record tournament message',
    (msg: string, time: string, callback: (status: string) => void) => {
      try {
...
function onRecordOrCheckGameInvitation(socket: Socket, userID: number) {
  socket.on(
    'record or check game invitation',
    (target_id: number, option: string, callback: (status: string) => void) => {
      const sender = activeUsers.get(userID);
      if (!sender) return callback('error');
      if (sender.invitations.sent.includes(target_id)) return callback('sent');
      if (sender.invitations.received.includes(target_id)) return callback('received');

      const receiver = activeUsers.get(target_id);
      if (!receiver)
        // double check if the user is still active
        return callback('offline');

      if (option === 'check')
        // check will be used to display proper state of the button and invitation banner
...
function onDeleteInvitation(socket: Socket, userID: number) {
  socket.on('delete invitation', (target_id: number, option: string, callback: () => void) => {
    ...
    if (option === 'received') {
      ...
      // This would be if you send invitation and then for some reason block this user
